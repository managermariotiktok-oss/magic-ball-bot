...
Примеры хорошего толкования:
— "Сегодня ты стоишь перед выбором, который кажется маленьким, но изменит многое. Не торопись — но и не медли. Кто-то ждёт твоего решения больше, чем ты думаешь."
— "Эта карта говорит: то, что ты отпустила месяц назад — было правильным. Сегодня придёт подтверждение. Вечером — неожиданный знак."
...
Ритуал должен быть:
— ВЫПОЛНИМЫМ прямо сегодня (не "медитируй 40 дней")
— КОНКРЕТНЫМ действием: "Напиши имя того, кого хочешь отпустить, и сожги бумагу", "Надень что-то красное", "Позвони тому, о ком думаешь с утра", "Положи монету под порог"
— С объяснением магического смысла (1 предложение)
...
async def get_prediction(period: str, mood: dict) -> str:
    prompt = (
        ...
        "Пример стиля: «Сегодня кто-то думает о тебе прямо сейчас. Не тот, кого ты ожидаешь. "
        "Старое сообщение которое ты боялась отправить — отправь его сегодня. "
        "Вечер принесёт неожиданный разговор который изменит многое.» "
        ...
        "Пример стиля: «Эта неделя начнётся с внутреннего сопротивления — но к среде что-то сдвинется. "
        "Деньги придут не оттуда откуда ждёшь. "
        "Четверг — особый день, не планируй его слишком жёстко. "
        "К воскресенью ты будешь удивлена тем как всё сложилось.» "
        "Не добавляй прощание в конце."
    )
    ...
    r = await groq_client.chat.completions.create(
        ...
        messages=[
            ...
            {"role": "user",   "content": (
                ...
                "Ответь ПРЯМО на этот конкретный вопрос — не уклоняйся, не говори общих слов. "
                "Дай ответ как будто ты действительно видишь ситуацию: что происходит, что будет, что нужно сделать. "
                "Назови конкретное действие или конкретный совет. "
                "Если вопрос про человека — скажи что-то конкретное про него. "
                "Если про деньги — назови источник или срок. "
                "Если про выбор — скажи однозначно. "
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    await message.answer(
        f"✨ <i>Звёзды выровнялись...</i>\n"
        f"🔮 <i>Шар пробуждается...</i>\n"
        f"👁 <i>Я вижу тебя, <b>{name}</b>...</i>\n\n"
        f"{SEP_TODAY}\n\n"
        f"<b>Судьба привела тебя сюда не случайно.</b>\n\n"
        f"<i>В глубине хрустального шара уже клубится туман.\n"
        f"Тысячи нитей твоей судьбы переплетаются прямо сейчас.\n"
        f"Спроси — и шар ответит.</i>\n\n"
        f"{SEP_END}\n\n"
        f"<i>🆓 Бесплатно каждый день: предсказание на сегодня, карта дня, 3 вопроса\n"
        f"⭐ За звёзды: предсказание на неделю, ритуал, безлимит</i>",
...
async def prediction_today(message: Message, state: FSMContext):
    ...
    if not has_unlimited(uid) and usage["today"] >= FREE_LIMITS["today"]:
        await message.answer(
            ...
            f"{SEP_PAY}\n\n"
            f"<i>Ты уже получил своё бесплатное предсказание на сегодня.\n"
            f"Но за звёзды шар откроет тебе больше ✨</i>\n\n"
            f"<i>Купи безлимит и получай все предсказания без ограничений 30 дней.</i>",
...
async def prediction_week(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        f"Открой завесу судьбы за <b>{PRICES['week']} звёзд Telegram</b> ✨</i>"
    )
...
async def ritual_of_day(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        f"Открой его за <b>{PRICES['ritual']} звёзд Telegram</b> ✨</i>"
    )
...
async def ask_prompt(message: Message, state: FSMContext):
    ...
    if unlimited:
        ...
        footer = f"<i>⭐ Бесплатные вопросы закончились — следующий за {PRICES['question']} звёзд</i>"
...
async def handle_question(message: Message, state: FSMContext):
    ...
    if not has_unlimited(uid) and usage["questions"] >= FREE_LIMITS["questions"]:
        ...
        await message.answer(
            ...
            f"Но за звёзды шар откроет тебе больше ✨\n\n"
